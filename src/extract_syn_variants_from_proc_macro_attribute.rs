pub fn extract_syn_variants_from_proc_macro_attribute(
    ...
    let generate_postgresql_crud_additional_http_status_codes_error_variant_path = format!("{proc_macro_name_lower_case}::{proc_macro_attribute_name}");
    let additional_http_status_codes_error_variants_attribute = proc_macro_helpers::get_macro_attribute::get_macro_attribute(
        &ast.attrs,
        &generate_postgresql_crud_additional_http_status_codes_error_variant_path,
    ...
    match additional_http_status_codes_error_variants_attribute.path.segments.len() == 2 {
        true => {
            let first_ident = &additional_http_status_codes_error_variants_attribute.path.segments.first().unwrap_or_else(|| {
                panic!("{proc_macro_name_camel_case_ident_stringified} {generate_postgresql_crud_additional_http_status_codes_error_variant_path} additional_http_status_codes_error_variants_attribute.path.segments.get(0) is None")
            }).ident;
            ...
            if let false = additional_http_status_codes_error_variants_attribute_path == possible_additional_http_status_codes_error_variants_attribute_path {
                panic!("{proc_macro_name_camel_case_ident_stringified} {generate_postgresql_crud_additional_http_status_codes_error_variant_path} {possible_additional_http_status_codes_error_variants_attribute_path} is not {generate_postgresql_crud_additional_http_status_codes_error_variant_path}")
            }
        },
        false => panic!("{proc_macro_name_camel_case_ident_stringified} {generate_postgresql_crud_additional_http_status_codes_error_variant_path} no {generate_postgresql_crud_additional_http_status_codes_error_variant_path} path")
    }
    ...
    additional_http_status_codes_error_variants_attribute_tokens_without_brackets_stringified.split(";").collect::<Vec<&str>>()
        .iter().fold(std::vec::Vec::<syn::Variant>::new(), |mut acc, element| {
            let element_derive_input: syn::DeriveInput = syn::parse(
                ...
                .unwrap_or_else(|_| panic!("{proc_macro_name_camel_case_ident_stringified} {generate_postgresql_crud_additional_http_status_codes_error_variant_path} {element} {}", proc_macro_helpers::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
                .into()
            ).unwrap_or_else(|e| {
                panic!("{proc_macro_name_camel_case_ident_stringified} {generate_postgresql_crud_additional_http_status_codes_error_variant_path} parse additional_http_status_codes_error_variants_attribute_tokens failed {e}");
            });
        ...
        data_enum
                ...
                panic!("{proc_macro_name_camel_case_ident_stringified} {generate_postgresql_crud_additional_http_status_codes_error_variant_path} does not work on enums!");
            };
