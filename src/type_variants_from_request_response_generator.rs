pub fn type_variants_from_request_response_generator(
    ...
    //     let enum_with_serialize_deserialize_logic_mapped_token_stream = type_variants_from_request_response
    //             .iter()
    //             .map(
    //                 |(
    //                     _,
    ...
    //                     },
    //                     None => {
    //                         acc.insert(&element.0.error_variant_attribute, vec![&element.0.error_variant]);
    //                     }
    //                 }
    //                 acc
    //             },
...
pub fn type_variants_from_request_response<'a>(
    ...
    let http_status_code_quote_token_stream = error_variant_attribute.to_http_status_code_quote();
    let fields_name_mapped_into_token_stream = error_variant_fields
        .iter()
        .map(|element| {
            let field_name_token_stream = &element.field_name;
            quote::quote! {#field_name_token_stream}
        ...
        .collect::<std::vec::Vec<proc_macro2::TokenStream>>();
    let fields_anonymous_types_mapped_into_token_stream = error_variant_fields
        .iter()
        .map(|element| {
            let field_name_token_stream = &element.field_name;
        ...
        .collect::<std::vec::Vec<proc_macro2::TokenStream>>();
    let try_operation_token_stream = {
        let fields_mapped_into_token_stream = error_variant_fields.iter().map(|element| {
