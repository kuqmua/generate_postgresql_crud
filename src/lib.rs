...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {//todo in few cases rows affected is usefull. (update delete for example). if 0 afftected -maybe its error? or maybe use select then update\delete?(rewrite query)
    ...
    let (
        ...
        //todo generate trait
        let operation_payload_element_try_from_operation_payload_element_with_serialize_deserialize_snake_case_token_stream = 
        
        proc_macro_helpers::naming_conventions::PayloadElementTryFromPayloadElementWithSerializeDeserializeSnakeCaseTokenStream::payload_element_try_from_payload_element_with_serialize_deserialize_upper_camel_case_token_stream(&operation);
        
        generate_payload_element_try_from_payload_element_with_serialize_deserialize_snake_case_token_stream(
