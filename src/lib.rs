...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {//todo in few cases rows affected is usefull. (update delete for example). if 0 afftected -maybe its error? or maybe use select then update\delete?(rewrite query)
    ...
    let ident_column_token_stream = {
        let ident_column_stringified = format!("{ident}Column");
        ident_column_stringified.parse::<proc_macro2::TokenStream>()
        .unwrap_or_else(|_| panic!("{proc_macro_name_camel_case_ident_stringified} {ident_column_stringified} {}", proc_macro_helpers::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
    };
    let column_token_stream = {
        ...
        quote::quote! {
            ...
            pub enum #ident_column_token_stream {
                #(#column_variants),*
            }
            impl std::fmt::Display for #ident_column_token_stream {
    ...
    let deserialize_ident_order_by_token_stream = {
        ...
        quote::quote!{
            fn #deserialize_ident_order_by_lower_case_name_token_stream<'de, D>(
                deserializer: D,
            ) -> Result<#crate_server_postgres_order_by_order_by_token_stream<#ident_column_token_stream>, D::Error>
            where
                                ...
                                Some(offset_slice_next_comma_index) => {
                                    match offset_slice.get(0..offset_slice_next_comma_index) {
                                        Some(possible_column) => match {
                                            ...
                                            #ident_column_token_stream::from_str(possible_column)
                                        } {
                                            Ok(column) => column,
                                            Err(e) => {
        ...
        let struct_token_stream = {
            ...
            quote::quote!{
                ...
                pub struct #ident_order_by_wrapper_name_token_stream(
                    ...
                    pub #crate_server_postgres_order_by_order_by_token_stream<#ident_column_token_stream>,
                );
            }
        };
                            ...
                            Some(index) => match index.checked_add(column_equal_str.len()) {
                                Some(offset) => match string_deserialized.get(offset..) {
                                    Some(offset_slice) => match offset_slice.find(split_inner_url_parameters_symbol) {
                                        Some(offset_slice_next_comma_index) => {
                                            match offset_slice.get(0..offset_slice_next_comma_index) {
                                                Some(possible_column) => match #ident_column_token_stream::from_str(possible_column) {
                                                    Ok(column) => column,
                                                    Err(e) => {
            ...
            let payload_token_stream = {
                ...
                quote::quote!{
                    ...
                    pub struct #operation_payload_camel_case_token_stream {
                        ...
                        pub #order_by_token_stream: #crate_server_postgres_order_by_order_by_token_stream<#ident_column_token_stream>,
            ...
            let payload_with_serialize_deserialize_token_stream = {
                ...
                quote::quote!{
                    ...
                    pub struct #operation_payload_with_serialize_deserialize_camel_case_token_stream {
                        ...
                        #order_by_token_stream: #crate_server_postgres_order_by_order_by_token_stream<#ident_column_token_stream>,
