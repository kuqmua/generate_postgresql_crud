...
pub fn generate_postgresql_crud(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let column_select_url_token_stream = {
        let column_select_url_ident_token_stream = {
            let column_select_url_ident_stringified = format!("{ident}ColumnSelectUrl");
    ...
    let column_select_json_token_stream = {
        let column_select_json_ident_token_stream = {
            let column_select_json_ident_stringified = format!("{ident}ColumnSelectJson");
            column_select_json_ident_stringified.parse::<proc_macro2::TokenStream>()
            .unwrap_or_else(|_| panic!("{proc_macro_name_ident_stringified} {column_select_json_ident_stringified} {}", proc_macro_helpers::global_variables::hardcode::PARSE_PROC_MACRO2_TOKEN_STREAM_FAILED_MESSAGE))
        };
        let column_select_json_struct_token_stream = {
            ...
            quote::quote! {
                #[derive(Debug, serde::Serialize, serde::Deserialize, Clone, strum_macros::Display)]
                pub enum #column_select_json_ident_token_stream {
                    #(#column_select_json_variants_token_stream),*
                }
            }
        };
        let generate_get_query_token_stream = {
            ...
            quote::quote!{
                impl crate::server::postgres::generate_get_query::GenerateGetQuery for #column_select_json_ident_token_stream {
                    fn generate_get_query(&self) -> std::string::String {
                        match self {
                            #(#generate_get_query_variants_token_stream),*
                        }
        ...
        let impl_default_token_stream = {
            ...
            quote::quote! {
                impl std::default::Default for #column_select_json_ident_token_stream {
                    fn default() -> Self {
                        Self::#default_select_variant_ident_token_stream
                    }
                }
        ...
        let from_option_self_token_stream = {
            quote::quote! {
                impl std::convert::From<Option<Self>> for #column_select_json_ident_token_stream {
                    fn from(option_value: Option<Self>) -> Self {
                        match option_value {
                            Some(value) => value,
                            None => Self::default(),
        ...
        let options_try_from_sqlx_row_token_stream = {
            ...
            quote::quote! {
                impl #column_select_json_ident_token_stream {
                    fn options_try_from_sqlx_row<'a, R: sqlx::Row>(
                        &self,
                        row: &'a R,
                    ) -> sqlx::Result<#struct_options_ident_token_stream>
        ...
        quote::quote! {
            #column_select_json_struct_token_stream
            #generate_get_query_token_stream
            #impl_default_token_stream
            #from_option_self_token_stream
            #options_try_from_sqlx_row_token_stream
    ...
    //     }
    // };
    println!("{column_select_url_token_stream}");
    println!("--------------------------------");
    println!("{column_select_json_token_stream}");
    let gen = quote::quote! {
        ...
        #column_select_json_token_stream
